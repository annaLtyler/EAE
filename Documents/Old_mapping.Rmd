...
This analysis was copied from an old html in which
the strain-average mapping and the individual mapping
matched closely. After this date, these two mappings
diverged. I am re-copying this code to see if I can
trace the divergence to any particular point.
...
results.dir <- here("Results", "Old_Results_Testing")
```
...
This analysis was run with `zero.treatment` the CDS measurement.
...
cds <- read.csv(here("Data", "CDS_April_2023.csv"), strip.white = TRUE)
incidence.files <- list.files(here("Data"), pattern = "incidence", full.names = TRUE)
...
batch2keep <- setdiff(u_batch, "X170")
u_batch <- u_batch[which(u_batch %in% batch2keep)]
...
The following heatmap shows how many individuals are in each strain by sex combination.
...
Each day of the study, each mouse is given a disease 
CDS score between 0 and 5, with lower numbers indicating 
less disease. The cumulative disease score (CDS) for a 
given mouse is the sum of these scores over the course 
of the study.
...
There are many mice that never display measurable 
symptoms and have cumulative disease scores of 0. 
This kind of floor effect causes problems with 
statistical tests that assume a normal distribution.

If exclude.zeros is set to TRUE, these 0 values will 
be excluded from CDS mapping.

In this analysis exclude.zeros is set to: `r exclude.zeros`.
...
The distribution of CDS is shown below.


```{r cds_dist2, results = "asis"}
...
The individuals were run in multiple batches, 
which can cause systematic biases in the data. 
We need to correct any batch effects that we find 
before we map.
...
The following boxplots show the batch values for the B6 animals. 
These show that the batches had an increase in CDS across the batches.

```{r b6_batch}
b6.idx <- which(factor.cds[,1] == "C57BL6")
batch.idx <- apply(cds.covar[,1:3], 2, function(x) intersect(which(x == 1), b6.idx))
b6.batches <- lapply(batch.idx, function(x) num.cds[x,])

par(mfrow = c(2,2))
b6.by.batch <- vector(mode = "list", length = ncol(num.cds))
names(b6.by.batch) <- colnames(num.cds)
for(i in 1:ncol(num.cds)){
    b6.by.batch[[i]] <- lapply(b6.batches, function(x) x[,i])
    boxplot(b6.by.batch[[i]], main = colnames(num.cds)[i])
}
...
To correct for batch effects we did the following:

1. Calculate the overall mean CDS for B6 mice across 
    the entire experiment
2. Calculate the mean CDS B6 mice in each batch.
3. Adjust all mice in each batch based on the difference 
    in means in the B6. Subtract off the mean deviation, 
    and divide by the standard deviation.
4. Male and female B6 mice do not have a sex difference 
    in CDS, and adjusting by the two sexes separately could 
    inadvertently introduce a sex effect into the other strains, 
    so we use overall means rather than sex-specific means.

### Age at D0

Another one of our covariates is Age.at.D0. The following plots 
show the relationship between Age.at.D0 and each CDS.
...
### Remove batch and age effects

Adjust each measure of CDS for batch. Subtract off 
the mean and divided by the standard deviation for 
the CDS measurements in each trait.
...
#for each batch
adj.cds <- num.cds
for(b in 1:nrow(batch.mean)){
    batch.idx <- which(cds.covar[,b] == 1)
    for(j in 1:ncol(num.cds)){
        adj.cds[batch.idx,j] <- (num.cds[batch.idx,j] - batch.mean[b,j])/batch.sd[b,j]
    }
}
```
...
for(i in 1:ncol(adj.cds)){
    cds.idx <- which(num.cds[,i] != 0)
    adj.cds[cds.idx,i] <- adjust(adj.cds[cds.idx,i,drop=FALSE], 
        cds.covar[cds.idx,"Age.at.D0",drop=FALSE])
}
```

Rank Z normalize the CDS scores.
...
Get the CC genotypes from the qtl2 github website, 
convert to genoprobs, and calculate kinship matrices.
...
If specified, add genoptypes for 129, NOD, and B6, 
so we can use them in mapping.
...
The following plots show the trait distributions for 
each strain. Each plot is ordered by overall strain 
mean.

For each strain, the dots are split and colored by sex. 
Each point represents one mouse. The horizontal line 
shows the mean value for that sex. This helps show where 
there might be sex effects in individual strains.
...
For the most part, there are no sex differences, The only 
strain for which there might be a real sex difference is CC046. 
Could this large difference be enough to cause the weird 
sex-specific interactions we see below?
...
The following plot shows that global CDS is dominated by 
classical disease. It also shows that CDS in classical 
...
### CDS by individual allele effects

```{r thresh}
allele.thresh = c(5,6,6)
```

The following plots show the allele effects for 
the largest QTL for each trait.
...
The following tables show LOD peak intervals 
for the top 3 peaks from each trait.

```{r top_peaks, results = "asis"}
top.n <- 3
...
## Map with Strain Average (Sex Additive)
...
and kinship matrices and duplicated each row for the two 
sexes.
...
```{r map_avg}
cds.avg.scans.sex.add <- map_pheno(mean.genoprobs, mean.pheno, mean.K, 
    exclude.zeros = FALSE, addcovar = mean.covar,
    results.file = file.path(results.dir, "scan1_results_cds_avg.RDS"))

if(run_permutations){
    cds.avg.scans.sex.add.perm <- perm_scans(mean.genoprobs, mean.pheno, mean.K,
        cds.avg.scans.sex.add, groups = NULL, exclude.zeros = FALSE, nperm = nperm,
        results.file = file.path(results.dir, "scan1perm_results_cds_avg.RDS"))
}
```

### LOD Traces {.tabset .tabset-fade .tabset-pills}

The following plots show the LOD traces for the strain 
averages of CDS and the significance thresholds.

```{r avg_lod, results = "asis"}
...
if(run_permutations){
    ...
    for(i in 1:length(cds.avg.scans.sex.add)){
        ...
        plot(cds.avg.scans.sex.add[[i]], map = map)
        cat("\n\n")
    }
}
```
...
```{r avg_allele, results = "asis"}
cds.avg.coef.sex.add <- allele_coef(map.geno = mean.genoprobs, 
    map.pheno = mean.pheno, map.k = mean.K, scan.results = cds.avg.scans.sex.add,
    addcovar = mean.covar,
...
We also used sex as an interacting covariate when mapping CDS.

```{r sex_int}
cds.avg.scans.sex.int <- map_pheno(mean.genoprobs, mean.pheno, mean.K, 
...
if(run_permutations){
    ...
    cds.avg.perm.sex <- perm_scans(map.geno = mean.genoprobs, 
        map.pheno = mean.pheno, map.k = mean.K,
    ...
    cds.avg.perm.sex.int <- perm_scans(map.geno = mean.genoprobs, 
        map.pheno = mean.pheno, map.k = mean.K, max.exp.lod = 40,
...
The following plots show the two LOD traces plotted 
on the same plot with significance thresholds.

```{r add_int_overlay, results = "asis"}
for(i in 1:length(cds.avg.scans.sex.int)){
    cat("####", names(cds.avg.scans.sex.int)[i],  "\n")
    if(run_permutations){
        ymax <- max(c(cds.avg.scans.sex.int[[i]][,1]), fdr_summaries_sev_avg_int[i,1:2])*1.05
    }else{
...
The figures below show the LOD scores for the 
additive model subtracted off the interactive 
model. The horizontal line is the maximum of the 
permuted values of the additive model subtracted 
off the maximum of the permuted values of the 
interactive model. This isn’t exactly the significance 
threshold for a locus that significantly interacts 
with sex, but it gives us a rough idea of where any 
interacting loci are most likely to reside.

```{r add_int_diff, results = "asis"}
for(i in 1:length(cds.avg.scans.sex.add)){
    cat("####", names(cds.avg.scans.sex.add)[i], "\n")
...
## Map Incidence with Strain Average

We also want to map incidence at the strain level. 
The incidence matrices are split by sex and 
...
u_strain <- unique(just.strain)
strain.idx <- match(u_strain, just.strain)
strain.idx <- strain.idx[which(!is.na(strain.idx))]

for(i in 1:length(mean.genoprobs)){
    strain.genoprobs[[i]] <- mean.genoprobs[[i]][strain.idx,,]
    rownames(strain.genoprobs[[i]]) <- u_strain
    strain.K[[i]] <- mean.K[[i]][strain.idx, strain.idx]
    rownames(strain.K[[i]]) <- colnames(strain.K[[i]]) <- u_strain
}
```

```{r inc_scan}
norm.inc <- lapply(incidence, function(x) apply(x, 2, function(y) rankZ(y)))


incidence.scan <- lapply(norm.inc, function(x) map_pheno(strain.genoprobs, x, 
    strain.K, exclude.zeros = FALSE, 
    results.file = file.path(results.dir, "scan1_results_incidence.RDS")))

if(run_permutations){
    for(i in 1:length(incidence)){
        incidence.perm <- perm_scans(strain.genoprobs, incidence[[i]], strain.K,
            incidence.scan[[i]], groups = NULL, exclude.zeros = FALSE, nperm = nperm,
            results.file = file.path(results.dir, paste0("scan1perm_results_", names(incidence)[i], ".RDS")))
    }
}
```

### Incidence LOD traces

```{r inc_lod}
#pdf("~/Desktop/test.pdf")
if(run_permutations){
    fdr_summaries_incidence <- vector(mode = "list", length = length(incidence))
    names(fdr_summaries_incidence) <- names(incidence)
    for(i in 1:length(incidence.scan)){
        fdr_summaries_incidence[[i]] <- plot_fdr_fun(incidence.scan[[i]], incidence.perm[[i]],
        tab.level = "####")
    }
}else{
    for(i in 1:length(incidence.scan)){
        cat("####", names(incidence.scan)[i], "{.tabset .tabset-fade .tabset-pills}\n")
        for(j in 1:length(incidence.scan[[i]])){
            cat("#####", names(incidence.scan[[i]])[j], "\n")
            plot(incidence.scan[[i]][[j]], map = map)
            cat("\n\n")
        }
        cat("\n\n")
    }
