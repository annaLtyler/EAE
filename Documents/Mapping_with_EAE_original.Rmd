...
*Note*
This workflow contains all the analyses that we did with 
the EAE data. The other mapping workflow contains a subset
of the analyses here for clarity and analysis run time.
...
overall.k.file <- here("Data", "kinship_overall_nonames.RDS")
...
### LOD Traces: Incidence With Sex as an Additive Covariate {.tabset .tabset-fade .tabset-pills}

```{r plot_perm_add, warning = FALSE, fig.height = 5, fig.width = 9, results = "asis"}
#pdf("~/Desktop/test.pdf")
if(run_permutations){
    fdr_summaries_incidence_sex_add <- plot_fdr_fun(inc.scan.sex.add, 
    incidence.sex.add.perm, tab.level = "####")
}else{    
    ymax <- max(sapply(inc.scan.sex.add, function(x) max(x[,1])))*1.05
    for(i in 1:length(inc.scan.sex.add)){
        cat("####", names(inc.scan.sex.add)[i], "\n")
        plot(inc.scan.sex.add[[i]], map = map, main = names(inc.scan.sex.add)[i], 
            ylim = c(0, ymax))
        cat("\n\n")
    }
}
...
### LOD Intervals: Incidence with Sex as Additive {.tabset .tabset-fade .tabset-pills} 

The following tables show LOD peak intervals for the top 3 peaks from 
each trait.


```{r inc_lod_peaks_add, results = "asis"}
for(ph in 1:length(inc.scan.sex.add)){
    cat("####", names(inc.scan.sex.add)[ph], "\n")
    peak.table <- find_peaks(inc.scan.sex.add[[ph]], map = map, prob = 0.95)
    lod.order <- order(peak.table[,"lod"], decreasing = TRUE)
    top.peaks <- peak.table[lod.order[1:top.n],3:ncol(peak.table)]
    rownames(top.peaks) <- NULL
    print(kable(top.peaks))
...
### LOD Intervals: Incidence with Sex as a Covariate {.tabset .tabset-fade .tabset-pills} 

The following tables show LOD peak intervals for the top 3 peaks from 
each trait.


```{r inc_lod_peaks_int, results = "asis"}
for(ph in 1:length(inc.scan.sex.int)){
    cat("####", names(inc.scan.sex.int)[ph], "\n")
    peak.table <- find_peaks(inc.scan.sex.int[[ph]], map = map, prob = 0.95)
    lod.order <- order(peak.table[,"lod"], decreasing = TRUE)
    top.peaks <- peak.table[lod.order[1:top.n],3:ncol(peak.table)]
    rownames(top.peaks) <- NULL
    print(kable(top.peaks))
...
eae.peak <- find_peaks(inc.scan.overall$EAE, map = map, threshold = 5)
