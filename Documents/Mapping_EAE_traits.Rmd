...
```{r results_dir}
results.dir <- here("Results", paste0("Zeros_", strsplit(zero.treatment, " ")[[1]][1]))
...
u_batch <- u_batch[-which(u_batch == "X170")]
...
b6.batch.idx <- apply(cds.covar[,1:3], 2, function(x) intersect(which(x == 1), b6.idx))
...
#change mean calculations to exclude zeros conditionally
mean.pheno <- matrix(NA, nrow = nrow(strain.by.sex), ncol = ncol(num.cds))
colnames(mean.pheno) <- colnames(num.cds)
for(i in 1:nrow(strain.by.sex)){
    ...
    group.mean[which(is.na(group.mean))] <- 0
...
```{r plot_with_sig, fig.width = 9, fig.height = 5, results = "asis"}
