...
split.model = FALSE #if true, we look fit two models, one to severity
...
top.n <- 3
for(ph in 1:length(severity.avg.scans)){
    cat(names(severity.avg.scans)[ph], "\n")
...
## Map Severity by Strain Average with Sex as Interacting Covariate
...
severity.avg.perm.sex.int <- perm_scans(map.geno = mean.genoprobs.sex, 
    map.pheno = mean.pheno.sex, map.k = mean.K.sex, max.exp.lod = 40,
    scan1.results = severity.avg.scans.sex, addcovar = covar.sex, intcovar = covar.sex,
    groups = perm.groups, split.model = FALSE, nperm = nperm,
    results.file = here("Results", "scan1perm_results_severity_avg_sex_int.RDS"))

```

### Sex as additive {.tabset .tabset-fade .tabset-pills}
...
### Sex as interactive {.tabset .tabset-fade .tabset-pills}
...
### Interactive and Additive Together
...
for(i in 1:length(severity.avg.scans.sex)){
    plot(severity.avg.scans.sex.int[[i]], map = map, col = "red", 
        main = names(severity.avg.scans.sex)[i])
