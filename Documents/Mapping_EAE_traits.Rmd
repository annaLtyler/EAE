...
for(i in 1:length(cds.avg.scans.sex.int)){
    ...
    if(run_permutations){
        ymax <- max(c(cds.avg.scans.sex.int[[i]][,1]), cds.avg.perm.sex[i,1:2])*1.05
    }else{
    ...
    if(run_permutations){
        abline(h = cds.avg.perm.sex[i,1:2], lty = 1:2, col = "red")
    }
    plot(cds.avg.scans.sex.add[[i]], map = map, add = TRUE)
    if(run_permutations){
        abline(h = cds.avg.perm.sex[i,1:2], lty = 1:2, col = "darkblue")
    }
    cat("\n\n")
}
