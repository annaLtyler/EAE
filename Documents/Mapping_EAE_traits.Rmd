...
inc.file.date = "October_2023" #date for incidence csv files to use in format month_year
...
for(i in 1:nrow(cc.chr)){
    png(file.path("~/Desktop/CC_geno", paste0(gsub("/", "-", rownames(cc.chr)[i], fixed = TRUE), "_Chr", h2.chr, "_geno.png")), width = 3, height = 8,  units = "in", res = 300)
    ...
    par(mar = c(4,0,4,4))
    plot(c(1, dim(mat)[2]), c(1, dim(mat)[1]), type = "n", axes = FALSE, xlab = "", ylab = "", xlim = c(0.7, dim(mat)[2]+0.2), ylim = c(0.7, dim(mat)[1]+0.2), bg = "transparent")
    rasterImage(mat, xleft = 0.5, ybottom = 0.5, xright = dim(mat)[2]+0.5, ytop = dim(mat)[1]+0.5, interpolate = FALSE, bg = "transparent")
    mtext(rownames(cc.chr)[i], side = 3)
    dev.off()

    all.col[[idx]] <- mat
    idx = idx + 1
    spacer.mat  <- matrix("white", nrow = dim(mat)[1], ncol = 4)
...
plot_coefCC(global.coef, map = map, scan1_output = global.lod,
    ylim = c(-2, 2), col = strain.colors, xlim = c(103, 119),
...
plot_coefCC(eae4.coef, map = map, scan1_output = eae4.lod, xlim = c(0,60),
