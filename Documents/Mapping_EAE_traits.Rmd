...
trait.name <- "EAE"
trait.idx <- which(names(inc.scan.overall) == trait.name)
max.qtl <- which.max(inc.scan.overall[[trait.idx]][,1])
max.marker <- rownames(inc.scan.overall[[trait.idx]])[max.qtl]

#test a random marker
#max.marker <- sample(rownames(inc.scan.overall[[trait.idx]]), 1)

marker.pos <- find_markerpos(map, max.marker)
marker.geno <- pull_genoprobpos(strain.genoprobs, marker = max.marker)

common.strains <- intersect(rownames(overall.inc), rownames(marker.geno))
annot.df <- data.frame(overall.inc[common.strains,trait.idx,drop=FALSE])

trait.order <- order(annot.df[,1])
ordered.names <- common.strains[trait.order]
pheatmap(marker.geno[ordered.names,], annotation_row = annot.df,
    cluster_rows = FALSE, cluster_cols = FALSE, 
    main = paste(trait.name, "Chr", marker.pos[,"chr"], ":", 
    signif(marker.pos[,"pos"], 2), "Mb"))

model <- fit1(round(marker.geno, 2), overall.inc[,trait.idx,drop=FALSE])
model$lod
barplot(model$coef)

strain.effects <- apply(round(marker.geno[common.strains,], 2), 2, function(x) annot.df[which(x > 0),1])
strain.order <- order(sapply(strain.effects, function(x) median(x, na.rm = TRUE)))
boxplot(strain.effects[strain.order], col = CCcolors[strain.order])
abline(h = 0)
```
