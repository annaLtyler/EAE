...
plot_pval_fun <- function(scan.results, perm.results, tab.level = "###"){
    ...
    for(ph in 1:length(scan.results)){
        ...
        maxy <- max(c(scan.results[[ph]][,1], 
            sig_summaries[finite.thresh], na.rm = TRUE))*1.1
        par(mar = c(4, 4, 4, 8))
        plot(scan.results[[ph]], map = map, main = names(scan.results)[ph], 
            ylim = c(0, maxy))
        abline(h = sig_summaries[ph,])
        abline(h = fdr_summaries[ph,])
        plot.dim <- par("usr")
        par(xpd = TRUE)
        text(x = plot.dim[2], y = sig_summaries[ph,], 
            labels = colnames(sig_summaries), cex = 0.5, adj = 0)
        par(xpd = FALSE)
        cat("\n\n")
    }
...
if(run_permutations){
    ...
    fdr_summaries_cds <- plot_fdr_fun(cds.scans, cds.perm, tab.level = "####")
...
plot_coefCC(global.coef, map = map, scan1_output = global.lod,
    ylim = c(-2, 2), col = strain.colors, xlim = c(101, 119),
...
if(run_permutations){    
    ...
    if(!file.exists(results.file)){
        ...
        perm.summ <- summary(incidence.perm, alpha = c(0.05, 0.1, 0.2))
...
if(run_permutations){
    ...
    for(ph in 1:length(inc.scan.overall)){
        ...
        ylim <- c(0, max(c(max(inc.scan.overall[[ph]]), max(perm.summ[,ph]))))
        plot(inc.scan.overall[[ph]], map = map, ylim = ylim)
        abline(h = perm.summ[,ph])
        cat("\n\n")
    }
}else{    
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
par(mar = c(4,8,4,4))
test <- plot_max_allele_effects("Global", mapping.geno = expanded.genoprobs, 
    mapping.pheno = mapping.pheno, mapping.K = exp.K, map = map, 
    scan.results = cds.scans)
...
par(mar = c(4,8,4,4))
plot_max_allele_effects("Global", mapping.geno = expanded.genoprobs, 
    mapping.pheno = mapping.pheno, mapping.K = exp.K, map = map, 
    scan.results = cds.scans)
dev.off()
```
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
test <- plot_max_allele_effects("EAE", mapping.geno = strain.genoprobs, 
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
test <- plot_max_allele_effects("AR.Major.Subtype", mapping.geno = strain.genoprobs, 
    mapping.pheno = overall.inc, mapping.K = strain.K, map = map, 
    scan.results = inc.scan.overall)
```
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
test <- plot_max_allele_effects("Remitting.EAE", mapping.geno = strain.genoprobs, 
    mapping.pheno = overall.inc, mapping.K = strain.K, map = map, 
    scan.results = inc.scan.overall)
```
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
test <- plot_max_allele_effects("Subtype..Classical.only", mapping.geno = strain.genoprobs, 
    mapping.pheno = overall.inc, mapping.K = strain.K, map = map, 
    scan.results = inc.scan.overall)
```
...
layout(matrix(c(1,2), ncol = 1), heights = c(1, 0.5))
test <- plot_max_allele_effects("Relapsing.Remitting", mapping.geno = strain.genoprobs, 
    mapping.pheno = overall.inc, mapping.K = strain.K, map = map, 
    scan.results = inc.scan.overall)
```
...
```{r allele_effects_inc_remitting_6}
chr <- 6
trait <- "Remitting.EAE"
...
```{r allele_effects_inc_classical_5}
chr <- 5
