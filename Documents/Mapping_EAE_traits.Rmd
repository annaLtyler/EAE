...
remove.zeros = TRUE #if true, we look only at severity given an animal has disease
                    #if false, we include zeros in the calculation of severity

nperm = 10000
```

```{r load_libraries, warning = FALSE}
all.packages <- c("pheatmap", "qtl2", "ggplot2", "abind", "igraph", "knitr")
...
symptoms, I think it makes more sense to map incidence
and severity separately. 

* *Severity*: The severity of disease when the animal
    does present with disease. The values of the non-zero 
    cumulative disease scores. Can be mapped at the individual 
    or strain level.
* *Incidence*: The fraction of animals in a strain with a
    non-zero cumulative disease score. Mapped at the
    strain level, or at the individual level as a binary
    trait.

### Incidence

The strain-level incidence for each trait is shown below.
...
The distributions are still far from normal, but aren't 
nearly as skewed as when we have all the 0's in there.

```{r incidence_dist, fig.height = 5, fig.width = 5}
inc.list <- lapply(1:ncol(strain.incidence), function(x) strain.incidence[,x])
names(inc.list) <- colnames(strain.incidence)
stripchart(inc.list, vertical = TRUE, pch = 16, col = "gray", method = "jitter")
```

### Severity {.tabset .tabset-fade .tabset-pills}

The distribution of severity of disease is shown below for 
each strain.

```{r severity, results = "asis", fig.width = 10, fig.height = 5}

plot_cds_by_strain <- function(scores, strain.labels, remove.zeros = TRUE,
...
for(cd in 1:ncol(num.cds)){
    ...
    plot_cds_by_strain(scores = num.cds[,cd], 
        strain.labels = factor.cds[,"Strain"], remove.zeros = remove.zeros,
...
cds.by.batch <- function(strain.names, batch.num = c(1:4), 
    ...
    remove.zeros = FALSE, plot.results = TRUE){
...
for(st in 1:length(u_strain)){
    ...
    if(length(batches.present) > 1){
        test <- cds.by.batch(u_strain[st], batch.num = batches.present, 
            remove.zeros = remove.zeros, plot.results = FALSE)
...
for(ch in 1:length(allele_freq)){
    ...
    for(a in 1:nrow(allele_freq[[ch]])){
        ...
        allele_freq[[ch]][a,], col = strain.colors[a], type = "l")
    }
    abline(h = 0)
    axis(1)
    axis(2)
...
trait_list <- function(strain.tables, trait.name, remove.zeros = TRUE, 
    ...
    if(remove.zeros){
        ...
        nonzero.idx <- which(unlist(strain.trait) != 0)
        strain.trait <- lapply(strain.trait, function(x) x[which(x != 0)])
    }else{
...
#strain.
trait_bars <- function(strain.tables, trait.name, remove.zeros = TRUE,
    col.by = NULL, plot.results = TRUE){
...
## Severity by Sex {.tabset .tabset-fade .tabset-pills}
...
plot_bars <- function(trait.name, remove.zeros){
...
sex.col <- as.numeric(as.factor(cds[,"Sex"]))
plot_bars("Classical", remove.zeros = remove.zeros)
```

### AR

```{r bars_ar, fig.width = 10, fig.height = 7}
plot_bars("AR", remove.zeros = remove.zeros)
```

### Global

```{r bars_global, fig.width = 10, fig.height = 7}
plot_bars("Global", remove.zeros = remove.zeros)
```
...
Since we are not seeing a reliable difference in the 
sexes, it is my opinion that we should adjust for sex
before doing the mapping and map with all individuals
at the same time.

```{r adjust_for_sex}
sex.adj.cds <- norm.cds
for(i in 1:ncol(norm.cds)){
    nonzero.idx <- which(num.cds[,i] > 0)
    sex.adj.cds[nonzero.idx,i] <- adjust(norm.cds[nonzero.idx,i,drop=FALSE], 
        matrix(as.numeric(as.factor(cds[nonzero.idx,"Sex"]))-1, ncol = 1))
}
```
...
trait.pairs <- pair.matrix(1:ncol(sex.adj.cds))
par(mfrow = c(2,2))
for(i in 1:nrow(trait.pairs)){
    trait1 <- trait.pairs[i,1]
    trait2 <- trait.pairs[i,2]
    nonzero.idx <- intersect(which(sex.adj.cds[,trait1] != 0), which(sex.adj.cds[,trait2] != 0))
    plot.with.model(sex.adj.cds[nonzero.idx,trait1], sex.adj.cds[nonzero.idx,trait2],
        xlab = colnames(sex.adj.cds)[trait1], ylab = colnames(sex.adj.cds)[trait2],
        main = paste(colnames(sex.adj.cds)[trait1], "vs.", colnames(sex.adj.cds)[trait2]))
}
```
...
mapping.pheno <- sex.adj.cds[pheno.idx,]
...
```{r mapping_fun}
map_pheno <- function(map.geno, map.pheno, map.K, 
    groups = NULL, remove.zeros = TRUE, 
...
perm_scans <- function(map.geno, map.pheno, map.k, scan1.results, 
    groups = NULL, nperm = 100, max.exp.lod = 20, n.founders = 8, 
    remove.zeros = TRUE, results.file = here("Results", "perm.RDS")){
        ...
        for(ph in 1:ncol(map.pheno)){
            ...
            if(!is.null(groups)){
                null_stats = scan1_groupperm_stats(map.geno, 
                    ...
                    scan1_out = scan1.results[[ph]][,1,drop=FALSE], cores = 4)
            }else{
...
plot_fdr_fun <- function(scan.results, perm.results){
...
allele_coef <- function(map.geno, map.pheno, map.k, scan.results, 
    lod.threshold, remove.zeros, chr.lod.file, chr.coef.file){

    peak.table <- lapply(scan.results, 
        function(x) find_peaks(x, map = map, threshold = lod.threshold))
...
plot_coef_scans <- function(coef.results, map.geno, map.pheno){
...
severity.perm <- perm_scans(expanded.genoprobs, mapping.pheno, exp.K,
    severity.scans, groups = names(expanded.idx), remove.zeros = remove.zeros, 
    nperm = nperm, results.file = here("Results", "scan1perm_results_severity.RDS"))
```

### Severity by individual LOD traces
...
The following plots show the allele effects for QTL with LOD 
scores greater than `r allele.thresh`.

```{r scan_coef, fig.width = 7, fig.height = 7, results = "asis"}
severity.coef <- allele_coef(expanded.genoprobs, mapping.pheno, 
    exp.K, severity.scans, allele.thresh, remove.zeros, 
...
plot_coef_scans(severity.coef, expanded.genoprobs, mapping.pheno)
#dev.off()
```

## Map Severity by Strain Average
...
severity.avg.perm <- perm_scans(mean.genoprobs, mean.pheno, mean.K,
    severity.avg.scans, groups = NULL, remove.zeros = FALSE, nperm = nperm,
...
```{r strain_avg_plots, fig.width = 9, fig.height = 5}
...
regression not convergins. So for now we will map incidence
...
incidence.perm <- perm_scans(mean.genoprobs, mean.pheno, mean.K,
    incidence.scan, groups = NULL, remove.zeros = FALSE, nperm = nperm,
