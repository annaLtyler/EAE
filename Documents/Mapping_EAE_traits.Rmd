...
for(i in 1:length(exp.K)){
    rownames(exp.K[[i]]) <- colnames(exp.K[[i]]) <- u_id
}
...
for(i in 1:length(expanded.genoprobs)){
    ...
    rownames(expanded.genoprobs[[i]]) <- u_id
}
...
perm_scans <- function(map.geno, map.pheno, map.k, scan1.results, 
    ...
    max.exp.lod = 100, n.founders = 8, exclude.zeros = TRUE, 
