# Compute summary stats of empirical null CDFs for permutations. 
#This is a helper function for computing FDRs.
#If groups is NULL, permutations will occur by individual
#Otherwise, groups should be a vector with repeated values
#indicating which individuals belong to different groups.
#In this case, individual groups (strains)
...
scan1_groupperm_stats = function(genoprobs, pheno, addcovar = NULL, kinship = NULL, 
                        intcovar = NULL, nperm = 100, breaks = seq(0, 1, 0.0001), 
                        scan1_out = NULL, groups = NULL, cores = 1, verbose = FALSE){
  ...
  for(n in 1:nperm){
    if(is.null(groups)){
        ...
        null_p = sample_groupperm_null(genoprobs, pheno, groups, 
        addcovar = addcovar, kinship = kinship, intcovar = intcovar, 
        cores = cores)
        #plot(-log10(null_p), map = map)
    }
  ...
  if(is.null(scan1_out)){
    scan1_out = scan1(genoprobs, pheno, kinship = kinship, addcovar = addcovar, 
