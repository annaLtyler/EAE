multilod.plot <- function(scan1.results, map, lod.thresh = 3, 
...
color.fun = c("linear", "exponential"), steepness = 1, use.pheatmap.colors = FALSE,
legend.height = 0.6){
    ...
    layout(matrix(c(1,1,2,0), ncol = 2), widths = c(1, 0.1), heights = c(legend.height, 1))
    par(mar = c(2, 6, 2, 0))
    imageWithText(t(val.mat), show.text = FALSE, col.names = NULL,
    col.scale = col.scale, color.fun = "exponential", exp.steepness = steepness,
    row.text.shift = row.name.shift, row.names = row.names, light.dark = "d",
    ...
    light.dark = "d", use.pheatmap.colors = use.pheatmap.colors)
